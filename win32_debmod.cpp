...
#define DEFAULT_BUFLEN 0x100000
#define DEFAULT_PORT "5000"

#define LOG_FUNC() msg("%s\n",__FUNCTION__)

char recvbuf[DEFAULT_BUFLEN];

int recvlen;

int connect_server(char *sendbuf,int sendlen,char *recvbuf,int *recvlen) 
{
    WSADATA wsaData;
...
ssize_t idaapi win32_debmod_t::dbg_read_memory(ea_t ea, void *buffer, size_t size, qstring * /*errbuf*/)
{
  ...
  */
  Json::Value sendroot,recvroot;
  Json::FastWriter writer;
  Json::Reader reader;
  sendroot["func"] = "dbg_read_memory";
  sendroot["ea"] = ea;
  sendroot["len"] = size;
  const std::string json_file = writer.write(sendroot);
  connect_server((char *)json_file.c_str(), json_file.length(), recvbuf, &recvlen);
  reader.parse(std::string(recvbuf,recvlen), recvroot);
  const std::string ret = recvroot["ret"].asString();
  unsigned char *buf = (unsigned char *)buffer;
  for (int i=0;i<size;i++) {
    buf[i]=index(ret[i*2])*16+index(ret[i*2+1]);
    printf("%02X ",buf[i]);
  }
  printf("\n");
  return size;
        ...
        qstring * /*errbuf*/)
{
  ...
  */
  ...
  Json::FastWriter writer;
  Json::Reader reader;
  sendroot["func"] = "dbg_read_registers";
  const std::string json_file = writer.write(sendroot);
  connect_server((char *)json_file.c_str(), json_file.length(), recvbuf, &recvlen);
  reader.parse(std::string(recvbuf,recvlen), recvroot);
  int sz = recvroot["ret"].size();
  for (int i=0; i<sz; i++){
    unsigned long long rval = recvroot["ret"][i].asUInt64();
    printf("%llX ",rval);
    values[i].ival = rval;
  }
  printf("\n");
